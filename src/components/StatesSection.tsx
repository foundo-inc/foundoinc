  ...
  <section className="py-20 md:py-28 bg-foreground relative overflow-hidden">
    ...
    <div className="container mx-auto px-4 relative z-10">
      <div className="text-center max-w-xl mx-auto mb-14">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-primary mb-3">Popular States</p>
        <h2 className="text-3xl md:text-4xl font-extrabold mb-4 font-display text-primary-foreground">
      ...
      <div className="grid md:grid-cols-3 gap-4 max-w-4xl mx-auto">
        {states.map((state, i) => (
          ...
          >
            {state.popular && (
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-primary-foreground text-primary text-[10px] font-bold px-3 py-1 rounded-full flex items-center gap-1 shadow-md">
                <Star className="h-3 w-3 fill-current" /> MOST POPULAR
              </div>
            )}
            <h3 className={`text-xl font-extrabold font-display mb-1 ${state.popular ? "text-primary-foreground" : "text-primary-foreground"}`}>{state.name}</h3>
            <p className={`text-2xl font-extrabold font-display mb-5 ${state.popular ? "text-primary-foreground" : "text-primary-foreground"}`}>
              {state.price}
              <span className="text-xs font-normal opacity-50 ml-1">one-time</span>
            </p>
            <ul className="space-y-2.5 mb-6">
              {state.benefits.map((b, j) => (
                <li key={j} className="flex items-start gap-2 text-xs">
                  <CheckCircle className={`h-3.5 w-3.5 shrink-0 mt-0.5 ${state.popular ? "text-primary-foreground/80" : "text-primary"}`} />
            ...
            <Button
              size="sm"
              className={`w-full rounded-full h-10 text-xs font-semibold transition-all duration-300 hover:-translate-y-0.5 ${
                state.popular
                  ? "bg-primary-foreground text-primary hover:bg-primary-foreground/90 shadow-lg"
                  : "bg-primary text-primary-foreground shadow-md shadow-primary/20"
              }`}
            >
              Start in {state.name} <ArrowRight className="ml-1.5 h-3.5 w-3.5" />
            </Button>
          </div>
        ))}
      </div>
