...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-background/80 backdrop-blur-2xl shadow-lg shadow-foreground/[0.03] border-b border-border/50' : 'bg-background/50 backdrop-blur-xl'}`}>
      ...
      {open && (
        <div className="md:hidden glass-card mx-4 mb-4 rounded-2xl p-5 space-y-1 shadow-2xl">
          {links.map(l => (
            <a key={l.label} href={l.href} className="block py-3 px-4 rounded-xl text-base font-medium text-foreground hover:bg-accent transition-colors" onClick={() => setOpen(false)}>
              {l.label}
            </a>
          ))}
