    ...
    <div className="container mx-auto relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6 lg:gap-8 max-w-5xl mx-auto">
        {states.map((state, i) => (
          ...
          >
            <div className={`absolute -top-3 md:-top-3.5 left-1/2 -translate-x-1/2 text-[10px] sm:text-xs font-bold px-3 sm:px-4 py-1 sm:py-1.5 rounded-full flex items-center gap-1 sm:gap-1.5 shadow-md whitespace-nowrap ${
              state.highlighted
                ? "bg-primary-foreground text-primary"
                : "bg-primary text-primary-foreground"
            }`}>
              <Star className="h-3 sm:h-3.5 w-3 sm:w-3.5 fill-current" /> {state.badge}
            </div>
            ...
            <p className="text-3xl sm:text-3xl md:text-3xl lg:text-4xl font-extrabold font-display mb-6 md:mb-8 text-primary-foreground">
              {state.price}
              <span className="text-sm md:text-base font-normal opacity-50 ml-1.5">Setup Fee</span>
            </p>
            <ul className="space-y-3 md:space-y-4 mb-7 md:mb-8 lg:mb-10">
              {state.benefits.map((b, j) => (
                <li key={j} className="flex items-start gap-3 text-sm sm:text-base md:text-base">
                  <CheckCircle className={`h-5 w-5 shrink-0 mt-0.5 ${state.highlighted ? "text-primary-foreground/80" : "text-primary"}`} />
                  <span className={state.highlighted ? "text-primary-foreground/80" : "text-primary-foreground/60"}>{b}</span>
                </li>
              ))}
            </ul>
            <Button
